...
SearchEngine *OptionParser::parse_cmd_line(
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg.compare("--sampled-Hoff-Trees") == 0) {
            ++i;
            leaves_to_sample = atoi(argv[i]);
	    cout<<"HoF Roots/leaves_to_sample:"<<leaves_to_sample<<endl;
        } else if (arg.compare("--Hoff_Root_F") == 0) {
            ++i;
            hoff_root_F = atoi(argv[i]);
        } else if (arg.compare("--print_F_bound_data") == 0) {
            ++i;
            F_bound_to_print = atoi(argv[i]);
	    cout<<"F_bound_to_print:"<<F_bound_to_print<<endl;
        } else if (arg.compare("--common_sampling_F_boundary") == 0) {
            ++i;
            common_sampling_F_boundary = atoi(argv[i]);
	    cout<<"F_bound_to_print:"<<F_bound_to_print<<endl;
        } else if (arg.compare("--Selec-heur") == 0) {
            ++i;
	    chosen_heurs= string(argv[i]);
	    cout<<"chosen_heurs:"<<chosen_heurs<<endl;
        } else if (arg.compare("--log_file") == 0) {
            ++i;
	    string temp(argv[i]);
	    log_file=temp;
	    cout<<"log_file:"<<log_file<<endl;
        } else if (arg.compare("--full_sampling") == 0) {
            ++i;
	    full_sampling=true;
	    cout<<"FULL SAMPLING,THIS IS NOT EFFICIENT,ONLY FOR CHECKING SELECTION QUALITY!:"<<log_file<<endl;
        } else if (arg.compare("--Phase") == 0) {
            ++i;
	    string temp(argv[i]);
	    if(temp.compare("SAMPLING")==0){
	      Current_RIDA_Phase=SAMPLING_PHASE;
	      cout<<"SAMPLING_PHASE"<<endl;
	    }
	    else if(temp.compare("SOLVING")==0){
	      Current_RIDA_Phase=SOLVING_PHASE;
	      cout<<"SOLVING_PHASE"<<endl;
	    }
	    else if(temp.compare("FIXED")==0){
	      Current_RIDA_Phase=FIXED_HEUR_COMB;
	      cout<<"FIXED_HEUR_COMB"<<endl;
	    }
	    else if(temp.compare("FITNESS")==0){
	      Current_RIDA_Phase=BEST_BY_FITNESS;
	      cout<<"BEST_BY_FITNESS"<<endl;
	    }
	    else if(temp.compare("RANDOM")==0){
	      Current_RIDA_Phase=RANDOM_HEUR_SELEC;
	      cout<<"RANDOM_HEUR_SELEC"<<endl;
	    }
	    else{
	      cout<<"Choose either \"SAMPLING\",\"SOLVING\",\"FITNESS\" or \"FIXED\" or \"RANDOM\"  Phase"<<endl;
	      exit(0);
	    }
        } else if (arg.compare("--Leaf_Reval_Percent") == 0) {
            ++i;
            leaf_reval_percent = atof(argv[i]);
	    cout<<"leaf_reval_percent:"<<leaf_reval_percent<<endl;
        } else if (arg.compare("--Degree") == 0) {
            ++i;
            Degree = atoi(argv[i]);
	    cout<<"Degree:"<<Degree<<endl;
        } else if (arg.compare("--use_saved_pdbs") == 0) {
	    use_saved_pdbs=true;
	    cout<<"USING SAVED_PDBS"<<endl;
        } else {
