...
void SpeedProgress::statistics() const {
...
int SpeedProgress::returnMaxF(vector<int> levels) {
      int max = levels.at(0);
...
int SpeedProgress::returnMinF(vector<int> levels) {
      int min = levels.at(0);
      for (int i = 0; i < levels.size(); i++) {
          if (min > levels.at(i)) {
             min = levels.at(i);
...
void SpeedProgress::dump_search_space() {
    search_space.dump();
}
...
void SpeedProgress::print_heuristic_values(const vector<int> &values) const {
