...
int EagerSearch::step() {
	...
	//Here we create our report about branching factor
        /*
	vector<int> nodes_expanded;
        vector<int> nodes_generated;
        int q = 0;
	for (map<int, int>::iterator it = nodes_expanded_by_level.begin(); it != nodes_expanded_by_level.end(); ++it) {
	    int fvalue = it->first;
            int nodesExpanded = it->second;
	    cout<<"fvalue -> nodesExpanded  "<<endl;
            cout<<fvalue<<" -> "<<nodesExpanded<<endl;
            nodes_expanded.insert(nodes_expanded.begin() + q, nodesExpanded);
            q =q + 1;
	}
        cout<<"\n\n";
        int p = 0;	
	for (map<int, int>::iterator it = nodes_generated_by_level.begin(); it != nodes_generated_by_level.end(); ++it) {
	    int fvalue = it->first;
            ...
            cout<<"fvalue -> nodesGenerated  "<<endl;
            cout<<fvalue<<" -> "<<nodesGenerated<<endl;
            nodes_generated.insert(nodes_generated.begin() + p, nodesGenerated);
            p = p + 1;
	}
        ...
        if (flag) {
	    for (int i = 0; i < p; i++) {
	         double result = (double)nodes_generated.at(i+1)/(double)nodes_expanded.at(i);
                 cout<<"bf["<<i<<"] = "<<result<<endl;
	    }
	}
        */ 
