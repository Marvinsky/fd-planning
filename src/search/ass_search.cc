...
void ASSSearch::output_problem_results() {
	ofstream outputFile;
...
void ASSSearch::statistics() const {
	...
	for (int i = 0; i < applicable_ops.size(); i++) {
		...
		succ_node.open(succ_h, nodecp, op);
		  ...
		  //the type already exists in the queue
		    ...
		    int  rand_100 = RanGen2->IRandom(0, 99);
...
pair<SearchNode, bool> ASSSearch::fetch_next_node() {
...
void ASSSearch::reward_progress() {
     open_list->boost_preferred();
}
/*
void ASSSearch::dump_search_space() {

}

*/
void ASSSearch::sample_frontier_now(int next_f_boundary) {
...
void ASSSearch::update_jump_statistic(const SearchNode &node) {	
...
/*
void ASSSearch::print_heuristic_values(const vector<int> &values) const {

}
*/

static SearchEngine *_parse_ass(OptionParser &parser) {
	cout<<" ______________________________"<<endl;
	cout<<"|  parse_ass - ass_search.cc     |"<<endl;
	cout<<" ______________________________"<<endl;
	...
	if (!parser.dry_run()) {
		...
		engine = new ASSSearch(opts);
		cout<<"engine = "<<engine<<endl;
	}	
	return engine;
}

static Plugin<SearchEngine> _plugin_ass("ass", _parse_ass);
