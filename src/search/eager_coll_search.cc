...
HST lsearch_space7(10000,1);
static bool time_limit_node_adjusted=false;

EagerCollSearch::EagerCollSearch(
    const Options &opts)
    : SearchEngine(opts),
...
void EagerCollSearch::output_problem_results(){
...
void EagerCollSearch::initialize() {
	...
	heuristics[i]->evaluate(*g_initial_state);
        ...
        Node2 node2(aux_h + initialNode.get_real_g(), initialNode.get_real_g()); 
...
void EagerCollSearch::statistics() const {
    search_progress.print_statistics();
    search_space.statistics();
}

int EagerCollSearch::step() {
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
	 ...
	 /*if(Current_RIDA_Phase==SOLVING_PHASE){
	    ...
	    cout<<"\tChild node h = "<<succ_node.get_h()<<",g = "<<succ_node.get_real_g()<<", f = "<<succ_node.get_h() + succ_node.get_real_g()<<" m&s h+g = "<<aux_h+succ_node.get_real_g()<<endl;
            ...
            Node2 node2(aux_h + succ_node.get_real_g(), succ_node.get_real_g());
...
int EagerCollSearch::returnMaxLevel(vector<int> levels) {
    int max = levels.at(0);
...
void EagerCollSearch::generateReport() {
        ...
        int depth = returnMaxLevel(levels);
        /*
        int sum = 0;
        for (map<Node2, int>::iterator iter = collector.begin(); iter != collector.end(); iter++) {
            Node2 n = iter->first;
        ...
        string directory = "mkdir /home/marvin/marvin/testss/"+heuristica+"/report/"+dominio+"/fdist/";
        ...
        outputFile<<"\t\t"<<fdist.c_str()<<"\n";
        outputFile<<"\ttotalniveles: "<<depth<<"\n";
        outputFile<<"\tthreshold: "<<depth<<"\n";
...
pair<SearchNode, bool> EagerCollSearch::fetch_next_node() {
...
void EagerCollSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void EagerCollSearch::dump_search_space() {
    search_space.dump();
}

void EagerCollSearch::update_jump_statistic(const SearchNode &node) {
	...
	//if(new_f_value==225&&first_sample){
	 if(Hoff_Root_Range>50&&F_boundary_time>3.0&&(first_sample)){
	  //if((leaves_to_sample)<Hoff_Root_Range&&(Current_RIDA_Phase==SAMPLING_PHASE)&&F_boundary_time>5.0)
	    ...
	    //HustSearch lsearch_space7(&opts2);
	    //HustSearch lsearch_space();
	    //lsearch_space7.select_best_estimated_heuristic_subset(&chosen_Hoff_Roots,orig_heuristics,heuristics);
     
	    cout<<"Memory before starting sampling:"<<get_peak_memory_in_kb()<<endl;
	    lsearch_space7.select_best_estimated_heuristic_subset(&search_space,&chosen_Hoff_Roots,orig_heuristics,heuristics,new_f_value,Hoff_Root_Range);
...
void EagerCollSearch::print_heuristic_values(const vector<int> &values) const {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    EagerCollSearch *engine = 0;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new EagerCollSearch(opts);
    }
...
static Plugin<SearchEngine> _plugin_astar("astar_coll", _parse_astar);
