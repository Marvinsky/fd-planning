#include "eager_dijkstra_search.h"
...
HST lsearch_space6(10000,1);
static bool time_limit_node_adjusted=false;

EagerDijkstraSearch::EagerDijkstraSearch(
    const Options &opts)
    : SearchEngine(opts),
...
void EagerDijkstraSearch::output_problem_results(){
...
void EagerDijkstraSearch::initialize() {
...
void EagerDijkstraSearch::statistics() const {
    search_progress.print_statistics();
    search_space.statistics();
}

int EagerDijkstraSearch::step() {
...
void EagerDijkstraSearch::generateReport() {
        ...
        string directory = "/home/marvin/marvin/test/"+heuristica+"/krereport/"+dominio+"/";
...
void EagerDijkstraSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void EagerDijkstraSearch::dump_search_space() {
    search_space.dump();
}

void EagerDijkstraSearch::update_jump_statistic(const SearchNode &node) {
	...
	//if(new_f_value==225&&first_sample){
	 if(Hoff_Root_Range>50&&F_boundary_time>3.0&&(first_sample)){
	  //if((leaves_to_sample)<Hoff_Root_Range&&(Current_RIDA_Phase==SAMPLING_PHASE)&&F_boundary_time>5.0)
	    ...
	    lsearch_space6.select_best_estimated_heuristic_subset(&search_space,&chosen_Hoff_Roots,orig_heuristics,heuristics,new_f_value,Hoff_Root_Range);
...
void EagerDijkstraSearch::print_heuristic_values(const vector<int> &values) const {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    EagerDijkstraSearch *engine = 0;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new EagerDijkstraSearch(opts);
    }
...
static Plugin<SearchEngine> _plugin_astar("astar_dijkstra", _parse_astar);
